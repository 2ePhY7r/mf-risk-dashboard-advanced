...
def load_and_transform_data():
    ...
    # ---------------------------------------------------------
    # 核心特征工程 9 大特征重建 (已添加 fillna 确保不报错)
    # ---------------------------------------------------------
...
# --- [新增] 专门用于解释性展示的非标准化模型 ---
@st.cache_resource
def get_interpretable_model(data):
    X = data[feature_cols]
    y = data['is_high_risk']
    # 不使用 scaler，确保系数对应原始数值单位
    model = LogisticRegression(max_iter=1000, random_state=42) 
    model.fit(X, y)
    return model

interpretable_lr = get_interpretable_model(df_fe)
